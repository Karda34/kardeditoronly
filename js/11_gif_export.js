...
const GIF_W         = 530;
const GIF_H         = 350;
const GIF_MAX_PLAY_SEC = 6.0;   // GIF: maximale Play-Phase-Dauer in Sim-Sekunden
const GIF_MAX_REAL_MS  = 18000; // GIF Failsafe: max Wartezeit in Echtzeitms

const MP4_MAX_PLAY_SEC = 8.0;   // MP4: maximale Play-Phase-Dauer in Sim-Sekunden
const MP4_MAX_REAL_MS  = 24000; // MP4 Failsafe: max Wartezeit in Echtzeitms
...
function _buildPlayData(nameRaw) {
  return {
    ...
    playType,                       // 'pass' | 'run' — so the play loads as the right type
    runCarrierId,                   // designated ball carrier for run plays
    motionOwnerId,
...
function generateAndSavePlayGif() {
  ...
  _drySavedSpeed = (typeof simSpeed !== 'undefined') ? simSpeed : 1;
  if (typeof simSpeed !== 'undefined') simSpeed = 1;
...
// ── JSON-only export ──────────────────────────────────────────────────────
function savePlayAsJson() {
  if (mode !== 'editor') { showToast('⚠ Only saveable in editor mode', 'info'); return; }
  const nameRaw      = (document.getElementById('playNameInput')?.value || '').trim();
  const playName     = nameRaw || 'play';
  const safeFilename = playName.replace(/[\\/:*?"<>|]/g, '_') + '.json';
  const jsonStr      = JSON.stringify(_buildPlayData(nameRaw), null, 2);
  const a = document.createElement('a');
...
function _mp4WatchLoop(ts) {
  if (!_dryRunning) return;
  const realElapsed = ts - _dryStartTs;
  const playDone = (typeof playPhaseTime !== 'undefined' && playPhaseTime >= MP4_MAX_PLAY_SEC);
  const timedOut = realElapsed >= MP4_MAX_REAL_MS;
